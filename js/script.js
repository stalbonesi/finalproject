...
$.getJSON('js/nynta2.geojson',function(data){
    var nynta2Data = data;
    geojsonLayer = L.geoJson(nynta2Data.features, {style: addStyle, onEachFeature: addInteractivity}).addTo(map);
});
...
info.update = function (props) {
    this._div.innerHTML = '<h4>NYC Residential Construction</h4>' +  (props ?
        '<b>' + props.NTAName + '</b><br />' + props.BoroName + '</b><br />' + props.ResUnits + ' Residential Units'
...
d3.csv("MN-bar-data.csv", function(error, data) {
  ...
  y.domain([0, d3.max(data, function(d) { return d.ResUnits; })]);
  ...
  svg.selectAll("bar")
    ...
    .enter().append("rect")
      ...
      .attr("y", function(d) { return y(d.ResUnits); })
      .attr("height", function(d) { return height - y(d.ResUnits); });
